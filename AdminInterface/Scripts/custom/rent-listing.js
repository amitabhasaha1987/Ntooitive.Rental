...
$(function () {
    ...
    if (typeof $.cookie('search-data') !== 'undefined') {
        ...
        var oTable = $('#rentpropertylisting').dataTable();
        oTable.fnFilter(search_value);
    }
});

function create_table() {
    $('#rentpropertylisting').dataTable({
        "bServerSide": true,
        "scrollX": true,
        "bDestroy": true,
        "sAjaxSource": "/property/rent-listing-ajax-handler",
        "bProcessing": true,
        "order": [[2, "asc"]],
        "aoColumns": [
...
function ManageProperty(mlsid) {
    window.location.href = "/property/rent/edit/" + mlsid;
}

function EditProperty(mlsid) {
    window.location.href = "/property/rent/new-Property/" + mlsid;
}

function DeleteProperty(mlsid) {
    $.ajax({
        url: "/Property/delete-RentListing",
        type: "POST",
        ...
        success: function (data, textStatus, jqXHR) {
            if (data.success) {
                ...
                var rentpropertylisting = $("#rentpropertylisting").dataTable({ bRetrieve: true });
                rentpropertylisting.fnDraw(true);
            } else {
